...
function makeBlue(original, output){
      ...
      // YOUR CODE GOES HERE
  //var original = document.getElementById("blueCanvas").src
...
function loadComposite(original, secondOne, output){
       // YOUR CODE GOES HERE
       for (var i=0; i<original.length; i+=4){
            output[i] = secondOne[i] + original[i];
            output[i+1]= secondOne[i+1] + original[i+1];
            output[i+2] = secondOne[i+2] + original[i+2];
            output[i+3] = secondOne[i+3] + original[i+3];
